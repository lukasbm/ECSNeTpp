    ...
    void ECSBuilder::executeAllocationPlan(cModule *parent)
    {
        ...
        for (it = allocationMap.begin(); it != allocationMap.end(); ++it)
        {
            ...
            for (it2 = tmp.begin(); it2 != tmp.end(); ++it2)
            {
                ...
                while (incomingConnectionsUsageMap[_parentName][_src->getFullName()] < incomingConnectionsCountMap[staskNameToCategoryMap[_src->getFullName()]])
                {
                ...
                while (outgoingConnectionsUsageMap[_parentName][_src->getFullName()] < outgoingConnectionsCountMap[staskNameToCategoryMap[_src->getFullName()]])
                {
